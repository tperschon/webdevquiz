...
function startQuiz() {
    ...
    // set the timer
    setTimer();
    // reset the number of correctly answered questions
    correctlyAnswered = 0;
    // hide the start screen, show the 
    startScreenDiv.setAttribute("style", "display: none;");
    questionAndAnswersDiv.setAttribute("style", "display: revert;");
}
...
function endQuiz() {
    ...
    stopTimer();
    // hide the question/answers screen and show the game over screen
    questionAndAnswersDiv.setAttribute("style", "display: none;")
    gameEndDiv.setAttribute("style", "display: revert")
}

// with the fed in click event, see what button is actually clicked, see
...
// answer question when user clicks an answer
